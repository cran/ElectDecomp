...
#'
#' @references Penades, A. and Pavia, J.M. *The decomposition of seats-to-votes distortion in multimember elections: mean, variance, malapportionment and participation*.
#'
...
#'  \item `aggregate.distortions`: A matrix of order 8x1 with the Loosemore-Handy index and its decomposition due to parties with and without representation as well as its cumulative decomposition step-by-step into mean effects, variance effects, malapportionment effects and turnout (and interaction) effects. The last row includes the MAL index.
#' }
#'
#' @export
#'
...
distortion <- function(votes,
  ...
  ## Outputs
  party.distorsions <- rbind(D_i, A_i, B_i, d_i, e_i, f_i, g_i)
  ...
  aggregate.distorsions <- rbind(D, Dr, Dwr, D.d, D.de, D.def, D.defg, MAL)
